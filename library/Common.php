...
if(isset($_GET[getParamAttriEnc()]) || isset($_POST[getParamAttriEnc()])){
	$encParams = isset($_GET[getParamAttriEnc()]) ? $_GET[getParamAttriEnc()] : $_POST[getParamAttriEnc()];
	parse_str(decryptIt($encParams),$GetParams);
	if(isset($_GET[getParamAttriEnc()])){
		unset($_GET[getParamAttriEnc()]);
		$_GET = array_merge($_GET,$GetParams);
	}
	if(isset($_POST[getParamAttriEnc()])){
		unset($_POST[getParamAttriEnc()]);
		array_merge($_POST,$GetParams);
	}
}
...
//hrcon Connection Class @-9703335A
class clsDBhrcon extends DB_Adapter
{
    function clsDBhrcon()
    {
        $this->Initialize();
    ...
    function Initialize()
    {
        ...
        $this->SetProvider($CCConnectionSettings["hrcon"]);
        parent::Initialize();
...
//CCLoginUser @0-CDC0F9B7
function CCLoginUser($login, $password)
{
    CCLogoutUser();
    $db = new clsDBhrcon();
    $SQL = "SELECT p_user_id, p_user_id, user_pwd FROM p_user WHERE user_name=" . $db->ToSQL($login, ccsText);// . " AND user_pwd=" . $db->ToSQL($password, ccsText);
    $db->query($SQL);
